...
$sum_test_result = "SELECT SUM(amount) as amount from patient_test where patient_id=$pid_account and status=0";
...
if (isset ($pid_account)) {
    if ($data['wallet'] > $amount['amount']) {
        $query = "update patient_test set payment_status='paid',status=1,payment_method='wallet' ,staff_id='$cds' where patient_id='$pid_account' and payment_status='notpaid' and status='0'";
